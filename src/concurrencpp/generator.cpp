...
concurrencpp::generator<int> infinite_counter()
{
    for (int i = 1;; ++i)
        co_yield i;
}

concurrencpp::generator<int> finite_counter(const int max)
{
    assert(max >= 1);
...
void benchmark()
{
    ankerl::nanobench::Bench().run("concurrencpp generator: infinite_counter", [] {
        int sum = 0;
    ...
    ankerl::nanobench::Bench().run("concurrencpp generator: finite_counter (iterator)", [] {
        auto gen = finite_counter(1000);
        int sum = 0;
    ...
    ankerl::nanobench::Bench().run("concurrencpp generator: finite_counter (range-for)", [] {
        int sum = 0;
